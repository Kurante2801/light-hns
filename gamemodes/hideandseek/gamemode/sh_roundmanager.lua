...
if SERVER then
    ...
    function GM:RoundCheck()
        timer.Simple(0.1, function()
            if self.RoundState == ROUND_ACTIVE then
                ...
                -- Advert last hider
                if not self.PlayedLastHiderSound and team.NumPlayers(TEAM_HIDE) == 1 then
                    local hider = team.GetPlayers(TEAM_HIDE)[1]
                    -- Refill stamina
                    hider:SetStamina(GAMEMODE.CVars.MaxStamina:GetInt())

                    -- Last hider trail
                    if self.CVars.HiderTrail:GetBool() and IsValid(hider) then
    ...
    function GM:RoundTimer(time)
        ...
        timer.Create("HNS.RoundTimer", time, 1, function()
            ...
            if self.CVars.TimeLimit:GetInt() > 0 and self.RoundState == ROUND_ACTIVE then
    ...
    function GM:RoundRestart()
        ...
        if team.NumPlayers(TEAM_HIDE) >= GAMEMODE.CVars.MinPlayers:GetInt() then
            ...
            if self.CVars.FirstSeeks:GetBool() and IsValid(self.FirstCaught) and self.FirstCaught:Team() ~= TEAM_SPECTATOR then
