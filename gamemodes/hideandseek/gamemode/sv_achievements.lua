...
hook.Add("Tick", "HNS.Achievements", function()
    if GAMEMODE.RoundState == ROUND_ACTIVE and lastSecond ~= GAMEMODE.TimeLeft then
...
hook.Add("HASPlayerTaunted", "HNS.Achievements", function(ply)
    if GAMEMODE.RoundState ~= ROUND_ACTIVE or ply:Team() ~= TEAM_HIDE then return end
    -- Conversionalist
...
hook.Add("HASRoundEndedTime", "HNS.Achievements2", function()
    ...
    for _, ply in pairs(team.GetPlayers(TEAM_HIDE)) do
        ...
        for _, a in pairs(team.GetPlayers(TEAM_HIDE)) do
            if ply ~= a and ply:GetPos():DistToSqr(a:GetPos()) <= 57600 then
                ccc = ccc + 1
            end
        end
...
hook.Add("PlayerSay", "HNS.Achievements", function(ply, text)
    -- Magic words
    if string.lower(text) == "tickle fight" or string.lower(text) == "ticklefight" then
